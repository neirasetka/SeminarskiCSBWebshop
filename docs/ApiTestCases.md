...
**Runnable script:** [`scripts/test-api-flows.ps1`](../scripts/test-api-flows.ps1) (login, catalog, cart, favorites, recommendations, giveaways, reports, RabbitMQ trigger).

> Route names below are the original PR #127 cases. Live API uses e.g. `POST /api/Users/Token` (not `/api/auth/login`) and `GET /api/Recommendation/GetRecommendedBags`.
